...
- Второе поколение — транзисторы 
  第二代 -- 晶体管
  – 5Э92б, 1964, самодиагностика, горячая замена, 500000 оп/c – БЭСМ-6, 1965 год, +ковейерная обработка, удаленное управление по телеф. Линиями .
  5Э92б，1964年，自诊断，热交换，500000操作/秒 - БЭСМ-6，1965年，+协变处理，通过电话远程控制。线路 .
- Третье поколение — интегральные схемы 
  第三代 -- 集成电路
  – Директива «Ряд», 1968 год, клонирование S/360, 1971 год — ЕС ЭВМ – Клоны РDP-11 
  指示 "行"，1968年，S/360的克隆，1971年--ЕС ЭВМ --PDP-11的克隆品 
- Четвертое поколение — сверхбольшие интегральные схемы 
  第四代--超大型集成电路 
  – Эльбрус — разработка по настоящее время
  厄尔布鲁士（处理器）--到目前为止的发展
  ![image](https://user-images.githubusercontent.com/90970853/194730370-ac7fe684-253e-4630-bb66-5008b6c568b1.png)
  ![image](https://user-images.githubusercontent.com/90970853/194730396-3d0c921d-da0f-46bb-91ed-93d890f72ecf.png)
  ![image](https://user-images.githubusercontent.com/90970853/194730397-8ca69489-1cf1-4851-b364-9108384a45d4.png)
  
#### 24. Архитектура многопроцессорных ЭВМ. Системный коммутатор. Архитектуры UMA и NUMA.  

**1. UMA:**
![image](https://user-images.githubusercontent.com/90970853/194730417-2e791303-236f-4c0a-ae93-2e0fbaae2e44.png)
![image](https://user-images.githubusercontent.com/90970853/194730420-9f65de91-5b4f-4b49-9dbe-d0a748e95a93.png)

**2. NUMA:**
![image](https://user-images.githubusercontent.com/90970853/194730426-3944c69e-848a-4470-8751-c798e393a7db.png)
...
#### 26. Адресуемая память, организация и временные диаграммы. Конструктивные особенности современной памяти.  
![image](https://user-images.githubusercontent.com/90970853/194731180-dd10c2c7-82df-499d-8374-f4e81550f1b9.png)
![image](https://user-images.githubusercontent.com/90970853/194731183-bda5a742-d891-4494-ac59-00ceb16ecb2c.png)
![image](https://user-images.githubusercontent.com/90970853/194731184-25bd9c36-d401-43e4-8c50-f0ef5063a7aa.png)
![image](https://user-images.githubusercontent.com/90970853/194731188-09a524c8-2b3d-444b-a5e9-8087bb20d39a.png)
...
- DDR4-2133 8192MB **PC4-17000** индекс производительности
  ...
  - пакетный режим (Burst Mode) – в нем проц. запрашивает данные из памяти не отдельными байтами, а в виде пакетов, состоящих из 32 или 64 бит, т.е. считывает 2 или 4 машинных слова за 1 такт;  
    突发模式（Burst Mode）— 在这种模式下，CPU不以单个字节的形式从内存中请求数据，而是以32或64位组成的数据包的形式请求数据，即每个时钟周期读取2或4个机器字。
  - чередование памяти (Interleaving Mode) – основано на том, что логические связанные байты располагаются в памяти друг за другом. В момент регенерации памяти она становится недоступной для процессора. Чтобы не было таких пауз в работе памяти, данные помещаются в различные микросхемы памяти. Таким образом, пока в одной из микросхем происходит регенерация данных, процессор в это время может считывать байты из другой микросхемы;  
    内存交错（Interleaving Mode）- 是基于逻辑链接的字节在内存中一个接一个地被放置的事实。在内存再生的时刻，内存变得无法被处理器访问。为了避免内存操作的这种暂停，数据被放置在不同的内存微芯片中。这意味着，当数据在一个芯片上再生的时候，处理器可以同时从另一个芯片上读取字节。
  - разбиение памяти на страницы (Paging Mode) – основан на том же принципе, что и предыдущий режим. Все ячейки с одинаковым адресом строки группируются в т.н. страницы. Поскольку адреса строк не изменяются, то это экономит время на поиск и считывание данных. Обычно память делится на страницы размером 512 и более байт;  
    内存分页、寻呼模式（Paging Mode）– 前一种模式相同。所有具有相同行地址的单元格被分组为所谓的页。由于行地址没有变化，这就节省了搜索和读取数据的时间。内存通常被分割成512字节或更多的页。
  - кэширование памяти – используется для ускорения доступа к данным, находящимся в RAM. Для этой цели между CPU и RAM ставится небольшая кэш-память, обычно в пределах 2 Мбайт, которая работает на частоте процессора, а значит при обращении к ней не требуются циклы ожидания.  
    内存缓存 - 用于加快对驻留在RAM中的数据的访问速度。为此，在CPU和RAM之间放置了一个小型的缓存，通常在2MB的范围内，它以CPU的频率运行，因此在访问它时不需要等待周期。
    ![image](https://user-images.githubusercontent.com/90970853/194731174-ddd79a89-0804-4e6e-a43c-7dd7bce51a58.png)


#### 27. Память, ориентированная на записи (блочная память). Организация дисковой памяти и памяти на магнитных лентах.  

![image](https://user-images.githubusercontent.com/90970853/194731199-7718ce02-7113-4407-a903-f730bab11790.png)
